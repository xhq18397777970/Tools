...
def execute_check():
    ...
    try:
        ...
        unreach_node_list=check_nodegroup_connectivity()
        if len(unreach_node_list)!=0:
            logger.warning("存在节点与salt-master连接失败")
            return jsonify({
                "data": "minion connect fail ,please retry",
                "unreach_node_number":len(unreach_node_list),
                "unreach_node_list":str(unreach_node_list)
            }), 207
